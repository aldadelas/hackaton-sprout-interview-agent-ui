 ...
 */
export function getSandboxTokenSource(appConfig: AppConfig, roomMetadata?: string) {
  return TokenSource.custom(async (options) => {
    const url = new URL(process.env.NEXT_PUBLIC_CONN_DETAILS_ENDPOINT!, window.location.origin);
    const sandboxId = appConfig.sandboxId ?? '';
    const shouldSetAgents = Boolean(appConfig.agentName) || Boolean(options.agentMetadata);
    const roomConfig = shouldSetAgents
      ? {
          agents: [
            {
